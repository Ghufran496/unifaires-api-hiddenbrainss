...
exports.initiateWithdrawal = useAsync(async (req, res, next) => {
  try {
    const { amount, userId, paypalEmail, amounttobededucted } = req.body; // Expecting amount, userId, and paypalEmail from frontend
    ...
    if (!amount || !userId || !paypalEmail) {
      return res
        ...
        .json(
          JParser(
            "Missing required parameters (amount, userId, paypalEmail)",
            false
          )
        );
    }
    ...
    console.log(
      "Initiating PayPal withdrawal for user: ",
      userId,
      amount,
      paypalEmail
    );
    ...
    return res
      ...
      .json(
        JParser(
          "PayPal withdrawal initiated successfully",
          true,
          payoutResponse
        )
      ); // Return PayPal's response
  } catch (error) {
    next(error);
  }
});
...
exports.handlePaypalWebhook = async (req, res) => {
  ...
  try {
    const paypalSignatureVerificationResult =
      await paypalServices.verifyPaypalWebhookSignature(
        req.headers,
        req.rawBody,
  ...
  try {
    ...
    return res
      .status(500)
      .send("Internal Server Error during PayPal webhook processing");
  }
};
